...
const defaults={
  ...
  weapons:{name:'未命名武器',base_item:'minecraft:diamond_sword',rarity:'common',level:1,attack_damage:10,attack_speed:1.6,critical_chance:.05,affix_pool:[],enabled:true},
  equipments:{name:'未命名裝備',base_item:'minecraft:iron_chestplate',equipment_slot:'chest',rarity:'common',level:1,armor:6,armor_toughness:0,health_bonus:0,affix_pool:[],enabled:true},
  ...
  monsters:{name:'未命名怪物',entity_type:'minecraft:zombie',health:20,damage:2,enabled:true},
...
function openDraft(key){
  state.key=key;state.editing=structuredClone(state.drafts[key]);if(['items','weapons','equipments','gems'].includes(state.kind)){state.editing.texture??='';state.editing.model??=''}$('draft-heading').textContent=key;$('draft-json').value=JSON.stringify(state.editing,null,2);
...
function renderObject(object,container,path){
  for(const [key,value] of Object.entries(object)){
    ...
    wrapper.append(input);if(key==='base_item'||key==='material'){const pick=document.createElement('button');pick.type='button';pick.className='button secondary material-pick';pick.textContent='從 Minecraft 圖示選擇';pick.addEventListener('click',()=>openMaterialPicker(next));wrapper.append(pick)}container.append(wrapper);
  }
}
...
async function uploadAsset(file,type){if(!file)return;if(file.size>5*1024*1024){toast('檔案超過 5 MB');return}const response=await fetch(`/api/asset?type=${type}&name=${encodeURIComponent(file.name)}`,{method:'POST',headers:{Authorization:`Bearer ${state.token}`},body:file});const payload=await response.json();if(!response.ok)throw new Error(payload.error||'上傳失敗');await loadAssets();renderVisualFields();toast('素材已上傳')}
...
$('editor-form').addEventListener('submit',event=>{event.preventDefault();const payload=checkJson();if(!payload)return;payload.key=state.key;$('save-status').textContent='儲存中…';api(`/api/draft?kind=${encodeURIComponent(state.kind)}&key=${encodeURIComponent(state.key)}`,{method:'PUT',body:JSON.stringify(payload)}).then(()=>reloadCurrent(state.key)).then(()=>{$('save-status').textContent='已儲存';toast('草稿已儲存')}).catch(fail)});
